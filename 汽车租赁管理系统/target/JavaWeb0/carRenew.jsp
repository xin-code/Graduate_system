...
<%
    ...
    if(op==2){
         ...
         */
        //查询原订单号
        Order order = getOrder(map.get("carid")[0]);
        ...
        renew=new TimeExtension(getRenewNum()+1,orderid,olddate,newdate,(int)(discount*Integer.parseInt(map.get("time")[0])
                *getCarPrice(order.getCarNumber()))) ;
    ...
    <script >
        ...
        function fun2() {
            ...
            if(pat1.test(carid) && pat2.test(time)){
                window.location.href="/carRenew.jsp?op=2&carid="+carid+"&time="+time
            }
            return false
        }
